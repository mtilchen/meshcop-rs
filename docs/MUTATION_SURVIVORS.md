...
- `crates/meshcop/src/meshcop/diag/decode.rs`
  ...
  the same value.
- `crates/meshcop/src/meshcop/coap.rs` CoAP header composition,
...
- `crates/meshcop/src/meshcop/coap.rs` option-header composition,
...
- `crates/meshcop-dtls/src/ecjpake/mod.rs` `Drop for EcJpakeParty` → `()`. The
...
- `crates/meshcop/src/commissioner/client/mod.rs`
  `commissioner_trace` → `()`. Tracing is a best-effort `eprintln!` gated on
  `MESHCOP_TRACE`. Its text and presence are deliberately not part of
