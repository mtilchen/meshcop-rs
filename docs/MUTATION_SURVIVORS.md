...
- `crates/mesh50-dtls/src/driver.rs` `DtlsReplayWindow::mark_seen`, `>` → `>=`
...
- `crates/mesh50/src/meshcop/coap.rs` option-header composition,
  `|` → `^`. The delta and length values occupy disjoint four-bit nibbles, so
  OR and XOR are identical.
- `crates/mesh50/src/commissioner/joiner.rs`
...
- `crates/mesh50-dtls/src/ecjpake/mod.rs` `Drop for EcJpakeParty` → `()`. The
...
- `crates/mesh50/src/commissioner/client/mod.rs`
  `commissioner_trace` → `()`. Tracing is a best-effort `eprintln!` gated on
  `MESH50_TRACE`. Its text and presence are deliberately not part of
