...
impl<'a> Collector<'a> {
    ...
    pub async fn collect_topology(
        &mut self,
        border_agent: String,
    ) -> mesh50::Result<TopologySnapshot> {
    ...
    pub async fn collect_node(&mut self, rloc16: u16) -> mesh50::Result<Node> {
    ...
    async fn collect_diag(
        ...
        tlvs: &[u64],
    ) -> mesh50::Result<Option<NetDiagData>> {
    ...
    async fn prime(&mut self, destination: Ipv6Addr) -> mesh50::Result<Option<NetDiagData>> {
    ...
    async fn keep_alive_before_request(&mut self) -> mesh50::Result<()> {
    ...
    fn network_info(&self, leader_rloc: u16, leader: &NetDiagData) -> mesh50::Result<NetworkInfo> {
