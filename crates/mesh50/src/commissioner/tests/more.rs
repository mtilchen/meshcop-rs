...
async fn commissioner_runs_petition_over_a_real_dtls_session() {
    use mesh50_dtls::{ContentType, DtlsRecord, test_support};
    ...
    let agent = tokio::spawn(async move {
        ...
        loop {
            ...
            for record in DtlsRecord::parse_datagram(&buf[..len]).unwrap() {
                ...
                let plaintext = mesh50_dtls::open_aes_128_ccm_8_record(
                    &record,
                    mesh50_dtls::RecordProtectionKey::new(keys.key_block.client_write_key),
                ...
                let protected = mesh50_dtls::protect_aes_128_ccm_8_record(
                    ContentType::ApplicationData,
                    1,
                    1,
                    mesh50_dtls::RecordProtectionKey::new(keys.key_block.server_write_key),
    ...
    let mut commissioner =
        Commissioner::connect(CommissionerConfig::pskc("mesh50", pskc), border_addr)
            .await
            .unwrap();
...
async fn clear_joiner_handler_drops_sessions_and_restores_raw_events() {
    ...
    let joiner = mesh50_dtls::ThreadDtlsHandshake::new(b"J01NME", &mut rng);
