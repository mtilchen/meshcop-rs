//! MeshCoP for Rust network diagnostics (`meshcop-netdiag`) for Thread.
//!
...
//!
//! ```text
//! meshcop-netdiag --border-agent 192.0.2.1:49154 --pskc <32-hex> topology --format markdown
//! meshcop-netdiag --border-agent 192.0.2.1:49154 --pskc <32-hex> node --rloc16 0xa400 --format json
//! ```
//!
...
struct Args {
    ...
    /// per-node diagnostic answer timeout, in seconds (default 4; must be less
    /// than half the configured commissioner keep-alive interval)
    #[argh(option, default = "DEFAULT_NODE_TIMEOUT_SECS")]
    node_timeout: u64,

    /// print sensitive fields (extended PAN ID, mesh-local prefix, IPv6
    /// addresses); redacted by default. Also honored via MESHCOP_SHOW_SECRETS
    #[argh(switch)]
    show_secrets: bool,
...
async fn run(args: Args) -> meshcop::Result<()> {
    ...
    let show_secrets = args.show_secrets || std::env::var_os("MESHCOP_SHOW_SECRETS").is_some();
...
fn render_node(node: &model::Node, format: OutputFormat, pretty: bool) -> meshcop::Result<String> {
    match format {
...
fn json<T: serde::Serialize>(value: &T, pretty: bool) -> meshcop::Result<String> {
...
fn resolve_pskc(pskc_arg: Option<&str>) -> meshcop::Result<[u8; 16]> {
...
fn parse_rloc16(value: &str) -> meshcop::Result<u16> {
