//! Joiner relay handling: driving the server-side DTLS session for relayed
//! joiner traffic.
...
use crate::{Result, meshcop};

use super::super::super::joiner::{JoinerSession, JoinerSessionEvent, joiner_id_from_iid};
...
impl Driver {
    ...
    pub(super) async fn handle_relay_rx(
        ...
        for (datagram, kek) in transmissions {
            let mut relay = meshcop::relay_tx_request_with_kek(
                0,
                Vec::new(),
                &iid,
                udp_port,
            ...
            self.assign_identity(&mut relay);
            self.link.send(&relay).await?;
        }
        Ok(())
    }
}
