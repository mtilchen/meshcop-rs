...
const COAP_RESET_ERROR: &str = "CoAP request was reset by the peer";
...
impl Commissioner {
    ...
    /// notifications are converted to queued events. A Reset of the expected
    /// request fails the exchange. Unmatched direct and proxied messages are
    /// dropped so delayed duplicate responses cannot poison a later exchange.
    ...
    async fn route_incoming(
        ...
        if let Some(udp_rx) = udp_rx {
            ...
            if let Some(request) = expected {
                ...
                if inner.is_reset_for(request.message_id) {
                    return Err(Error::InvalidState(COAP_RESET_ERROR));
                }
        ...
        if let Some(request) = expected {
            ...
            if incoming.is_reset_for(request.message_id) {
                return Err(Error::InvalidState(COAP_RESET_ERROR));
            }
...
mod tests {
    ...
    #[test]
    fn replacement_handshake_and_petition_fit_the_minimum_keepalive_interval() {
        use crate::commissioner::config::MIN_KEEPALIVE_INTERVAL;

        assert_eq!(DTLS_HANDSHAKE_TIMEOUT, Duration::from_secs(15));
        assert!(DTLS_HANDSHAKE_TIMEOUT + COAP_EXCHANGE_TIMEOUT < MIN_KEEPALIVE_INTERVAL);
        assert_eq!(Commissioner::EXCHANGE_TIMEOUT, COAP_EXCHANGE_TIMEOUT);
    }
