...
async fn connect_only_establishes_dtls_and_resign_closes_the_session() {
    with_test_deadline(async {
        ...
        let _border_agent_session = session.unwrap();
        let (commissioner, mut events) = connected.unwrap();
...
async fn scripted_harness_drives_petition_dataset_reads_events_and_resign() {
    with_test_deadline(async {
        ...
        let (commissioner, mut commissioner_events) =
            scripted_commissioner(script, [CommissionerEvent::DatasetChanged]).await;
...
async fn public_api_maps_protocol_errors_without_touching_live_network() {
    with_test_deadline(async {
        ...
        assert_eq!(pending_petition.status(), SessionStatus::Connected);

        let (rejected_petition, _rejected_petition_events) = scripted_commissioner(
            ScriptedMeshcopTransport::new([exchange(
                CommissionerOperation::Petition,
        ...
        assert_eq!(rejected_petition.status(), SessionStatus::Connected);
        ...
        let (accepted_with_echo, _accepted_with_echo_events) = scripted_commissioner(
            ScriptedMeshcopTransport::new([exchange(
                CommissionerOperation::Petition,
        ...
        assert!(accepted_with_echo.status().is_active());

        let (rejected_set, _rejected_set_events) = scripted_commissioner(
        ...
        let (delayed_duplicate, _delayed_duplicate_events) = scripted_commissioner(
            ScriptedMeshcopTransport::new([exchange(
                CommissionerOperation::Petition,
                [
        ...
        let (unsolicited_events, mut unsolicited_events_events) = scripted_commissioner(
            ScriptedMeshcopTransport::new([
                exchange(
                    CommissionerOperation::Petition,
                    [ScriptedResponse::petition_accept(0x1111)],
        ...
        for _ in 0..40 {
            assert_eq!(
                next_event(&mut unsolicited_events_events).await,
                Some(CommissionerEvent::DatasetChanged)
            );
        }
        assert_eq!(
...
async fn petition_requires_connected_inactive_commissioner_state() {
    with_test_deadline(async {
        ...
        let (active, _active_events) = scripted_commissioner(
            ScriptedMeshcopTransport::new([exchange(
                CommissionerOperation::Petition,
...
async fn public_commissioner_methods_are_table_driven() {
    with_test_deadline(async {
        let cases = [
            ...
            PublicCommissionerMethod::Subscribe,
