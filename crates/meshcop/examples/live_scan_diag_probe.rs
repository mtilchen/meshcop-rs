...
//!      `MESHCOP_MUTATE_OK=1`, since it makes devices transmit MLE
//!      Announce frames — and
//!   6. resigns.
//!
//! Usage:
...
//!
//! Pass `--show-secrets` (or set `MESHCOP_SHOW_SECRETS`) to print
...
async fn main() -> meshcop::Result<()> {
    ...
    let border_agent: SocketAddr = args
        ...
        .expect("border-agent host:port argument or MESHCOP_BORDER_AGENT required")
        .parse()
    ...
    let config = CommissionerConfig::from_dataset("meshcop-scan", &dataset)?;
...
async fn probe(
    ...
    if std::env::var_os("MESHCOP_MUTATE_OK").is_some_and(|value| value == "1") {
        ...
        println!("-- announce begin skipped (set MESHCOP_MUTATE_OK=1 to enable) --");
    }
...
fn print_diag_summary(data: &meshcop::meshcop::NetDiagData, show_secrets: bool) {
