use super::*;
use mesh50::commissioner::harness::{ScriptedExchange, ScriptedMeshcopTransport, ScriptedResponse};
use mesh50::commissioner::{CommissionerConfig, JoinerHandler};
use mesh50::meshcop::CommissionerOperation;
...
fn full_dataset_bytes() -> Vec<u8> {
    ...
    dataset.set_raw(mesh50::dataset::TLV_CHANNEL, vec![0, 0, 19]);
    dataset.set_raw(
        mesh50::dataset::TLV_CHANNEL_MASK,
    ...
    dataset.set_raw(mesh50::dataset::TLV_NETWORK_KEY, vec![0x42; 16]);
    dataset.set_raw(mesh50::dataset::TLV_NETWORK_NAME, b"cli-net".to_vec());
    dataset.set_raw(
        mesh50::dataset::TLV_PAN_ID,
        0xfaceu16.to_be_bytes().to_vec(),
    );
    dataset.set_raw(mesh50::dataset::TLV_PSKC, vec![0x24; 16]);
    dataset.set_raw(
...
async fn scheduled_keepalive_uses_the_configured_cadence_and_rearms() {
    ...
    assert!(matches!(
        interpreter
            ...
            .await
            .unwrap_err(),
        mesh50::Error::InvalidState("DTLS session is not established")
    ));
}
...
async fn rejected_scheduled_keepalive_disarms_the_timer() {
    ...
    assert!(matches!(
        ...
        mesh50::Error::InvalidState("scheduled keep-alive was rejected")
    ));
    assert_eq!(interpreter.keepalive_deadline(), None);
    assert_eq!(
...
async fn pending_scheduled_keepalive_disconnects_and_disarms() {
    ...
    assert!(matches!(
        interpreter.handle_scheduled_keepalive().await.unwrap_err(),
        mesh50::Error::InvalidState("scheduled keep-alive response is pending")
    ));
    assert_eq!(interpreter.keepalive_deadline(), None);
    assert_eq!(
        interpreter.commissioner.as_ref().unwrap().state(),
...
async fn failed_scheduled_keepalive_disconnects_and_disarms() {
    ...
    assert!(matches!(
        interpreter.handle_scheduled_keepalive().await.unwrap_err(),
        mesh50::Error::InvalidState("scripted MeshCoP exchange did not produce a response")
    ));
    assert_eq!(interpreter.keepalive_deadline(), None);
    assert_eq!(
        interpreter.commissioner.as_ref().unwrap().state(),
        ...
        [
            (
                ...
                vec![ScriptedResponse::content(vec![
                    mesh50::meshcop::TLV_BORDER_AGENT_LOCATOR,
                    2,
                    0x4c,
                    0x00,
                ])],
    ...
    let mut interpreter = active_interpreter(
        [
            ...
            (
                CommissionerOperation::GetCommissionerDataset,
                vec![ScriptedResponse::content(vec![
                    mesh50::meshcop::TLV_JOINER_UDP_PORT,
                    2,
                    0x03,
                    0xea,
                ])],
...
async fn commdataset_get_and_set_round_trip_json() {
    ...
    comm_dataset.set_raw(mesh50::meshcop::TLV_STEERING_DATA, vec![0xff]);
    let mut interpreter = active_interpreter(
        [
            (
                CommissionerOperation::GetCommissionerDataset,
...
async fn opdataset_get_projects_every_field_like_the_cpp_cli() {
    ...
    pending_dataset.set_raw(mesh50::dataset::TLV_NETWORK_NAME, b"cli-net".to_vec());
    pending_dataset.set_raw(
        mesh50::dataset::TLV_PENDING_TIMESTAMP,
        (2u64 << 16).to_be_bytes().to_vec(),
    );
    pending_dataset.set_raw(
        mesh50::dataset::TLV_DELAY_TIMER,
        60000u32.to_be_bytes().to_vec(),
    );
    let minimal = {
        let mut d = Dataset::default();
        d.set_raw(mesh50::dataset::TLV_NETWORK_NAME, b"min".to_vec());
        d.to_bytes().unwrap()
    };
    ...
    let mut interpreter = active_interpreter(
        [
            ...
            (
                ...
                vec![ScriptedResponse::content(vec![
                    mesh50::meshcop::THREAD_TLV_STATUS,
                    1,
                    0,
                ])],
            ),
        ...
        [
            (
                ...
                vec![ScriptedResponse::Coded {
                    code: mesh50::meshcop::CoapCode(0x84),
                    payload: Vec::new(),
                }],
            ),
            (
