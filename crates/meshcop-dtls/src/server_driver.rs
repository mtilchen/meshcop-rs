...
use crate::{
    ...
    driver::{
        ...
        SessionRole, SessionState, UnconnectedUdp, decode_alert_error, recv_records_before,
        recv_records_from_unbounded, recv_records_unbounded, send_records, sleep, with_timeout,
    },
    flight::take_record_sequence,
    is_client_finished_flight, open_aes_128_ccm_8_record, parse_unfragmented_handshake_messages,
    parse_unfragmented_handshake_record, renumber_epoch_zero_flight,
};
    ...
    D: DelayNs + Clone,
{
    ...
    let mut retransmit_timer = pin!(sleep(delay.clone(), schedule.timeout()));
    let mut duplicate_retransmissions = DuplicateRetransmitBudget::new();
    ...
    loop {
        let (records, _, _) =
            match recv_records_before(transport, peer, retransmit_timer.as_mut()).await {
                Ok(received) => received,
                Err(DriverError::Timeout) => {
                    renumber_epoch_zero_flight(&mut server_flight, &mut next_epoch_zero_record);
                    send_records(transport, local, peer, &server_flight).await?;
                    schedule.back_off();
                    retransmit_timer.set(sleep(delay.clone(), schedule.timeout()));
                    continue;
                }
        ...
        for record in records {
            match (record.header.epoch, record.header.content_type) {
                ...
                (1, ContentType::Handshake) => {
                    ...
                    // Without the ClientKeyExchange the record cannot be
                    // authenticated; drop it and wait for the retransmission.
                    let Some(keys) = key_material.as_ref() else {
                        continue;
                    };
                    // After the client's ChangeCipherSpec, a Finished that
                    // fails authentication means the keys differ (a PSK
                    // mismatch), so it ends the handshake with an alert; see
                    // the driver module docs.
