        ...
        match fault {
            ...
            FlightFault::ForwardAllButFirstRecord => {
                let remaining = records
                    ...
                    .ok_or(Error::Crypto("faulted DTLS flight is empty".into()))?;
                let partial = DtlsRecord::encode_datagram(remaining)?;
            ...
            FlightFault::ForwardAllButSecondRecord => {
                ...
                records.remove(1);
                let partial = DtlsRecord::encode_datagram(&records)?;
                socket.send_to(&partial, destination).await?;
            }
            FlightFault::ReverseRecords => {
                records.reverse();
                let reordered = DtlsRecord::encode_datagram(&records)?;
...
async fn dtls_server_ignores_invalid_records_before_cookie_exchange() -> crate::Result<()> {
    ...
    // An unparseable datagram from any host must not abort the acceptor.
    let stranger = tokio::net::UdpSocket::bind("127.0.0.1:0").await?;
    stranger.send_to(&[0x16, 0xfe, 0xfd], server_addr).await?;
    client_socket.send(&[0x16, 0xfe, 0xfd]).await?;
...
#[tokio::test]
async fn connect_discards_noise_before_the_server_finished() -> crate::Result<()> {
    let (client, server) = loopback_pair().await?;
    let noisy_server = tokio::spawn(async move {
        test_support::loopback_dtls_server(
            &server,
            &[0x42; 16],
            test_support::LoopbackEnd::NoiseBeforeFinished,
        )
        .await
    });
    let session =
        DtlsSession::connect(&client, &[0x42; 16], core::time::Duration::from_secs(2)).await?;
    let server_keys = noisy_server
        .await
        .expect("server task panicked")?
        .expect("noisy handshake returns keys");
    assert_eq!(
        session.key_material().master_secret,
        server_keys.master_secret
    );
...
#[tokio::test]
async fn ignored_datagrams_do_not_postpone_the_retransmission_timer() -> crate::Result<()> {
    // The peer keeps sending datagrams the client ignores more often than the
    // retransmission interval. The timer is armed once per flight, so the
    // ClientHello is still retransmitted on schedule.
    let noise_interval = crate::driver::DRIVER_INITIAL_RETRANSMIT_TIMEOUT / 5;
    let (client, server) = loopback_pair().await?;
    let client_task = tokio::spawn(async move {
        DtlsSession::connect(&client, &[0x42; 16], core::time::Duration::from_secs(2)).await
    });
    let first_hello = recv_one(&server).await?;
    let ignored = DtlsRecord::new(ContentType::ApplicationData, 0, 0, vec![0xaa])?.encode()?;
    let retransmission = async {
        let mut buffer = [0u8; crate::driver::MAX_DATAGRAM_SIZE];
        loop {
            tokio::select! {
                received = server.recv(&mut buffer) => break received.map(|length| buffer[..length].to_vec()),
                () = tokio::time::sleep(noise_interval) => {
                    server.send(&ignored).await?;
                }
            }
        }
    };
    let retransmitted_hello =
        tokio::time::timeout(core::time::Duration::from_secs(1), retransmission)
            .await
            .map_err(|_| crate::Error::Timeout("ClientHello was not retransmitted"))??;
    assert!(record_sequences_are_fresh(
        &first_hello,
        &retransmitted_hello
    ));
    assert_eq!(
        handshake_flight_fingerprint(&retransmitted_hello)?,
        handshake_flight_fingerprint(&first_hello)?
    );
    client_task.abort();
    Ok(())
}
