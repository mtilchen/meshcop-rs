...
use crate::{
    ContentType, DtlsClientHelloState, DtlsRecord, Error, HandshakeType, ThreadDtlsHandshake,
    ...
    driver::{
        ...
        recv_records_before, send_records, sleep, with_timeout,
    },
    flight::take_record_sequence,
    ...
    D: DelayNs + Clone,
{
    ...
    let mut retransmit_timer = pin!(sleep(io.delay.clone(), schedule.timeout()));
    loop {
        let (records, _, _) =
            match recv_records_before(io.transport, io.peer, retransmit_timer.as_mut()).await {
                ...
                Err(DriverError::Timeout) => {
                    ...
                    schedule.back_off();
                    retransmit_timer.set(sleep(io.delay.clone(), schedule.timeout()));
                    continue;
                }
        ...
        for record in records {
            match record.header.content_type {
                ...
                ContentType::Alert => {
                    return Err(decode_alert_error(&record.header, &record.payload).into());
                }
                _ => {}
            }
        }
    ...
    D: DelayNs + Clone,
{
    ...
    loop {
        ...
        if repeats_final_server_message(&records, server_flight) && duplicate_retransmissions.take()
        {
            *client_flight = build_client_finished_flight(
                client_key_exchange,
                client_finished,
        ...
        for record in records {
            match (record.header.epoch, record.header.content_type) {
                ...
                (1, ContentType::Handshake) => {
                    ...
                    // After the server's ChangeCipherSpec, a Finished that
                    // fails authentication means the keys differ (a PSKc
                    // mismatch), so it ends the handshake; see the driver
                    // module docs.
                    let Some(plaintext) = state.open_protected_record(&record)? else {
                        continue;
                    };
                ...
                (1, ContentType::Alert) => {
                    if let Ok(Some(plaintext)) = state.open_protected_record(&record) {
                        return Err(decode_alert_error(&record.header, &plaintext).into());
                    }
                }
                _ => {}
            }
...
/// Returns whether a datagram repeats the final message of the server's
/// previous flight (its ServerHelloDone).
///
/// Like mbedTLS, only the last message of the peer's previous flight triggers
/// a retransmission, so a server flight split across several datagrams
/// still triggers exactly one response per repetition.
fn repeats_final_server_message(
    records: &[DtlsRecord],
    server_flight: &[crate::HandshakeMessage],
) -> bool {
    let Some(final_message) = server_flight.last() else {
        return false;
    };
    records.iter().any(|record| {
        record.header.epoch == 0
            && record.header.content_type == ContentType::Handshake
            && parse_unfragmented_handshake_messages(record)
                .is_ok_and(|messages| messages.contains(final_message))
    })
}
...
mod tests {
    ...
    fn message(message_type: HandshakeType, message_seq: u16) -> HandshakeMessage {
        HandshakeMessage {
            message_type,
            message_seq,
            payload: Vec::new(),
    ...
    fn handshake_record(messages: &[&HandshakeMessage], epoch: u16) -> DtlsRecord {
        let mut payload = Vec::new();
        for message in messages {
            payload.extend(message.encode().expect("encode handshake message"));
        }
        DtlsRecord::new(ContentType::Handshake, epoch, 1, payload).expect("build record")
    }

    #[test]
    fn detects_a_repeated_final_server_message_in_whole_and_split_flights() {
        let server_hello = message(HandshakeType::ServerHello, 1);
        let key_exchange = message(HandshakeType::ServerKeyExchange, 2);
        let hello_done = message(HandshakeType::ServerHelloDone, 3);
        let flight = [
            server_hello.clone(),
            key_exchange.clone(),
            hello_done.clone(),
        ...
        let whole = handshake_record(&[&server_hello, &key_exchange, &hello_done], 0);
        assert!(repeats_final_server_message(
            core::slice::from_ref(&whole),
            &flight
        ));
        // A split retransmission triggers only on the datagram carrying the
        // final message.
        let first_half = handshake_record(&[&server_hello, &key_exchange], 0);
        let second_half = handshake_record(&[&hello_done], 0);
        assert!(!repeats_final_server_message(&[first_half], &flight));
        assert!(repeats_final_server_message(
            core::slice::from_ref(&second_half),
            &flight
        ));
    }

    #[test]
    fn ignores_records_that_cannot_repeat_the_server_flight() {
        let hello_done = message(HandshakeType::ServerHelloDone, 3);
        let flight = [hello_done.clone()];

        assert!(!repeats_final_server_message(
            &[handshake_record(&[&hello_done], 0)],
            &[]
        ));
        assert!(!repeats_final_server_message(
            &[handshake_record(&[&hello_done], 1)],
            &flight
        ));
        let wrong_content = DtlsRecord::new(
            ...
            0,
            1,
            hello_done.encode().expect("encode"),
        )
        .expect("build record");
        assert!(!repeats_final_server_message(&[wrong_content], &flight));
        let other_sequence = handshake_record(&[&message(HandshakeType::ServerHelloDone, 4)], 0);
        assert!(!repeats_final_server_message(&[other_sequence], &flight));
        let malformed = DtlsRecord::new(ContentType::Handshake, 0, 2, vec![0xff])
            .expect("build malformed handshake record");
        assert!(!repeats_final_server_message(&[malformed], &flight));
    }
}
